...
namespace ludusGestao.Eventos.Application.Validations.Local
{
    ...
    {
        public AtualizarLocalValidation(ILocalRepository repository, string idAtual)
        {
            RuleFor(x => x.Nome)
                ...
                {
                    var localExistente = await repository.BuscarPorId(idAtual);
                    if (localExistente != null && localExistente.Nome == nome)
                        return true; // Não mudou o nome, não precisa validar unicidade
                    return new LocalNomeUnicoSpecification(repository).IsSatisfiedBy(nome);
                })
